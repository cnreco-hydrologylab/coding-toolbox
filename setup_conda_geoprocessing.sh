...
#-----------------------------------------------------------------------------------------
# Script information
script_name='GEOPROCESSING - PYTHON3 LIBRARIES FOR PACKAGE - NAME TYPE - CONDA'
...
fp_env_tag_default='geoprocessing_env'
...
# conda create --yes --name $fp_env_folder_libraries -c conda-forge pyresample pygeobase
...
if [ -f $fp_env_file_requirements ] ; then
	...
	source activate $fp_env_folder_libraries
	conda install -c conda-forge python numpy pandas matplotlib scipy pip jupyter jupyterlab ipykernel openpyxl numpy scipy matplotlib gdal h5py cartopy pandas rasterio netCDF4 xarray dask pip rioxarray geopandas fiona
	...
	# pip install ascat, gldas pygeogrids pytesmo==0.13.4
	
	echo " ======> [2/2] PYTHON-PIP INSTALLATION ... DONE"
	
	# additional installations from source
	# cd module_folder/
	# python setup.py install
	# cd ../.. # back to starting directory if we moved...
...
echo "PATH=$fp_cwd/conda/bin:"'$PATH'"" >> $fp_env_file_reference
